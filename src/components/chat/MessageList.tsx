...
function getToolLabel(toolName: string, args: any, isDone: boolean): string {
  ...
  if (toolName === "str_replace_editor") {
    ...
    if (cmd === "str_replace" || cmd === "insert") return isDone ? `Edited ${filename}` : `Editing ${filename}`;
    if (cmd === "view") return `Reading ${filename}`;
    return isDone ? "File updated" : "Updating file";
  }
  if (toolName === "file_manager") {
    const cmd = args?.command;
    if (cmd === "rename") return isDone ? `Renamed ${filename}` : `Renaming ${filename}`;
    if (cmd === "delete") return isDone ? `Deleted ${filename}` : `Deleting ${filename}`;
    return isDone ? "File managed" : "Managing file";
  }
  return toolName;
}
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation":
                            ...
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-violet-100 rounded-lg text-xs font-medium border border-violet-200">
                                {isDone ? (
                                  <>
                                    <div className="w-2 h-2 rounded-full bg-emerald-500 flex-shrink-0"></div>
                                    <span className="text-violet-800">{getToolLabel(tool.toolName, tool.args, true)}</span>
                                  </>
                                ) : (
                                  <>
                                    <Loader2 className="w-3 h-3 animate-spin text-violet-600 flex-shrink-0" />
                                    <span className="text-violet-800">{getToolLabel(tool.toolName, tool.args, false)}</span>
                                  </>
                                )}
                              </div>
                            );
